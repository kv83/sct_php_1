...
echo 'Выберите сотрудника для редактирования<br><br>';
        ...
        $select_sql = "SELECT id, login, password FROM log_pass_sotrudnik";
        $result = mysqli_query($db, $select_sql);
        $row = mysqli_fetch_array ($result);
        $select_sql2 = " SELECT id, fio, dolzhnost FROM dannie_sotrudnik";
        $result2 = mysqli_query($db, $select_sql2);
        $row2 = mysqli_fetch_array ($result2);
        do
        {
            printf("<input type='radio' name='log_pass_sotrudnik'  value='%s'>%s %s %s %s <br/><br/>", $row['id'], $row['login'], $row['password'], $row2['fio'], $row2['dolzhnost']);
        }
